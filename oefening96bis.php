...
class Berichten{
    ...
    public function ToonBerichten(){
        $lijst= array();
        $sql = "select id, nickname, boodschap, datum from chatscript order by 4 DESC LIMIT 0,20";
...
if (isset($_GET["actie"]) && $_GET["actie"]=="toevoegen"){
    if (empty($_POST["nickname"]) && empty($_POST["boodschap"])){
        
        print 'gelieve een boodschap in te geven';
        }else {
        $bericht->NieuwBericht( $_POST["bericht"], $_POST["nickname"]);
        if (!isset($_SESSION["nick"])){ $_SESSION["nick"]=$_POST["nickname"]; }       
    }
}
...
<html>
    ...
    <body>
        ...
        <div class="chatbox">
            <form action="oefening96bis.php?actie=toevoegen" method="post">
                Bericht:<br><br>
                <?php 
                if (isset($_SESSION["nick"])){?>
                <input type="hidden" name="nickname" value="<?php echo $_SESSION["nick"];?>">
                <?php }else {?>
                <input type="text" name="nickname" placeholder="Nicknaam ?">
                <?php }?>
